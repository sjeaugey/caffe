...
__global__ void ring_sum_kernel(const int rank, const int nranks, 
  ...
  int align;
  for (align=2; align < max_delta_size && align < (nranks*2048); align <<= 1) {};
  align >>= 1;
  const int block_size = ((size+(gridDim.x*align)-1)/(gridDim.x*align)) * align; // upper(size/dim)
...
template<>
void multi_gpu_ring_sum<float>(const int rank, const int nranks, int *my_progress, float* red_data, float* my_data, int *next_progress, float* next_red_data, float* next_data, float factor, const int size, const int grid_dim, cudaStream_t stream) {
  ring_sum_kernel<float><<<grid_dim, COLL_NUM_THREADS, 0, stream>>>(
      rank, nranks, my_progress, red_data, my_data, next_progress, next_red_data, next_data, factor, size);
}

template<>
void multi_gpu_ring_sum<double>(const int rank, const int nranks, int *my_progress, double* red_data, double* my_data, int *next_progress, double* next_red_data, double* next_data, double factor, const int size, const int grid_dim, cudaStream_t stream) {
